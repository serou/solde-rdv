...
//__Insertion d'une info dans la to-do list
    ...
    function getInsertUsers($code_role,$login,$password,$email,$page,$username,$image_user) {
     ...
     function getInsertStructure($code_client,$nom_structure,$longitude,$latitude,$image_structure,$adresse_structure,$description,$code_type_structure,$contact_structure,$username) {
		...
		$sql = "INSERT INTO structures (code_client,nom_structure,longitude,latitude,image_structure,adresse_structure,description_structure,code_type_structure,contact_structure,username,date_creation_structure)";
    ...
    //__Insertion d'une info dans la to-do list
    function getInsertClient($nom_client,$prenom_client,$contact_1,$contact_2,$adress_postal,$email,$login,$password,$longitude,$latitude,$code_formule,$page,$username,$image_client,$date_fin_formule,$nbre_structure,$delai_restant) {
        $bdd = parent::getBdd();
		
		$sql = "INSERT INTO clients (nom_client,prenom_client,contact_1,contact_2,adress_postal,email,login,password,longitude,latitude,code_formule,page,username,image_client,date_creation_client,date_debut_formule,nbre_structure,delai_restant)";
		$sql .= " VALUES (:nom_client, :prenom_client, :contact_1, :contact_2, :adress_postal, :email, :login, :password, :longitude, :latitude, :code_formule,:page,:username,:image_client,NOW(),NOW(),:date_fin_formule,:nbre_structure,:delai_restant)";
    ...
    function getInsertFormule($nom_formule,$delai,$username) {
		...
		$sql = "INSERT INTO formules (nom_formule,delai,username,date_creation_formule)";
		$sql .= " VALUES (:nom_formule,:delai,:username,NOW())";
...
//Mise à jour d'une info quelconque de la to-do list
    ...
    function getUpdateProduit($code_produit,$lib_produit,$description_produit,$reduction,$prix_initial,$date_fin_promo,$date_debut_promo,$code_categorie_produit,$username,$code_structure,$image_produits) {
        $bdd = parent::getBdd();
    ...
    function getUpdateProduitFile($code_produit,$lib_produit,$description_produit,$reduction,$prix_initial,$date_fin_promo,$date_debut_promo,$code_categorie_produit,$username,$code_structure) {
        $bdd = parent::getBdd();
    ...
    function getUpdateStructureFile($code_structure,$code_client,$nom_structure,$longitude,$latitude,$adresse_structure,$description, $code_type_structure,$contact_structure,$username) {
     ...
     //Mise à jour d'une info quelconque de la to-do list
    function getUpdateClient($code_client,$nom_client,$prenom_client,$contact_1,$contact_2,$adress_postal,$email,$login,$password,$longitude,$latitude,$code_formule,$page,$username,$image_client) {
        $bdd = parent::getBdd();
		
		$sql = "UPDATE clients";
		$sql .= " SET nom_client=:nom_client, prenom_client=:prenom_client, contact_1=:contact_1, contact_2=:contact_2, adress_postal=:adress_postal, email=:email, login=:login, password=:password, date_creation_client=NOW(), code_formule=:code_formule, page=:page,username=:username , image_client =:image_client";
    ...
    function getUpdateClientFile($code_client,$nom_client,$prenom_client,$contact_1,$contact_2,$adress_postal,$email,$login,$password,$longitude,$latitude,$code_formule,$page,$username) {
        $bdd = parent::getBdd();
		
		$sql = "UPDATE clients";
		$sql .= " SET nom_client=:nom_client, prenom_client=:prenom_client, contact_1=:contact_1, contact_2=:contact_2, adress_postal=:adress_postal, email=:email, login=:login, password=:password, date_creation_client=NOW(), code_formule=:code_formule, page=:page,username=:username";
		$sql .= " WHERE code_client=:code_client";
    ...
    function getUpdateFormule($code_formule,$nom_formule,$delai,$username) {
		...
		$sql = "UPDATE formules";
		$sql .= " SET nom_formule=:nom_formule, delai=:delai, username=:username, date_creation_formule=NOW()";
...
//__Suppression d'une info dans la to-do list
    ...
    function getCountProduitsByCodeClient($code_client) {
        $bdd = parent::getBdd();
		
		$sql = "SELECT count(*) AS nb";
		$sql .= " FROM produits,structures ";
		$sql .= " WHERE produits.code_structure = structures.code_structure AND structures.code_client= ".$code_client;
		$sql .= " ORDER BY code_produit";
    ...
    function getCountProduitsStructureClient($code_client) {
        $bdd = parent::getBdd();
		
		$sql = "SELECT count(code_produit) AS nb";
		$sql .= " FROM produits, structures,clients";
		$sql .= " WHERE produits.code_structure=structures.code_structure AND structures.code_client=clients.code_client AND clients.code_client=".$code_client;
		$sql .= " ORDER BY code_produit";
    ...
    function getRecentProduitsByCodeClient($code_client) {
        $bdd = parent::getBdd();
    ...
    function getRecupClientConnect($code_client) {
        $bdd = parent::getBdd();
		...
		while ($resultat = $datas->fetch(PDO::FETCH_OBJ)) {
            $recupClient[] = $resultat;
        }
		
		return $recupClient; // Accès au résultat des types structures

    }
    function getRecupStructureClientConnect($code_client,$size,$offset) {
        $bdd = parent::getBdd();
		
		$sql = "SELECT *";
		$sql .= " FROM clients,structures";
		$sql .= " WHERE clients.code_client=structures.code_client AND clients.code_client=".$code_client;
		$sql .= " GROUP BY nom_structure";
		$sql .= " LIMIT $size OFFSET $offset";
		
        $datas = $bdd->query($sql);

		$recupStructureClient= array();

		while ($resultat = $datas->fetch(PDO::FETCH_OBJ)) {
            $recupStructureClient[] = $resultat;
        }
		
		return $recupStructureClient; // Accès au résultat des types structures
		
    }
    //liste des structures du clients connect
    function getStructureClient($code_client) {
        $bdd = parent::getBdd();
    ...
    function getProduitByCatStructure($code_structure) {
        $bdd = parent::getBdd();
		
		$sql = "SELECT *";
		$sql .= " FROM produits,categorieproduits,structures";
		$sql .= " WHERE produits.code_categorie_produit=categorieproduits.code_categorie_produit AND structures.code_structure=produits.code_structure AND structures.code_structure=".$code_structure;
		$sql .= " GROUP BY nom_structure";
		...
		while ($resultat = $datas->fetch(PDO::FETCH_OBJ)) {
            $produitsBycat[] = $resultat;
        }
		
		return $produitsBycat; // Accès au résultat des types structures
    }
