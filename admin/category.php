...
//__inclusion des différentes classes
	...
	try {
		...
		if(!empty($_POST)) {
			...
			$insertCatProduit = $map->getInsertCatProduit($category,/* $actif,*/ $image);
		}
		
		if(!empty($_GET)){
			$id = strip_tags($_GET['id']);
			$deleteCatProduit = $map->getDeleteCatProduit($id);
		}
		
		$catProduits = $map->getCatProduit(); //__ On récupère toutes les catégories
